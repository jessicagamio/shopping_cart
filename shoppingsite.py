...
def add_to_cart(melon_id):
    ...
    session["cart"] = session.get("cart", {})
    # check if melon_id in cart, if not, add it, either way increment by 1
    session["cart"][melon_id] = session["cart"].get(melon_id, 0) + 1

    # retrieve melon name from melon_types dict
    melon_name = melons.melon_types[melon_id].common_name

    flash(f"Added {melon_name} to cart!")
...
def process_login():
    ...
    if get_by_email(email) != "No customer with that email":

        if get_by_email(email).password == entered_password:
            session["logged_in_customer_email"] = email

            flash("Login successful!")
            return redirect("/melons")

        elif get_by_email(email).password != entered_password:
